...
//	fprintf(outputVariantResultFile, "\n#chr\tstart\tstop\tvariant_call_type\tvariant_call_id\tAverage_align_score\tsample_id\tSV_attributes\tzygosity\tlikelihood\tscores\tCoverage\tAverage_align_confidence\n");
	...
	variant[i].start, variant[i].end, tempIndel, variant[i].isSignal?".site":"", variant[i].sizeExt, 0.0, inputTrio, variant[i].size, tempH, variant[i].likelihood, variant[i].ratio,variant[i].support, variant[i].oppoSupp);
		numberOfSupportedSV++;
	}
}
