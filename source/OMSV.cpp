...
struct opticalMapType{
	...
	{////
        ...
        void print(FILE* targetFile){
                ...
                for (LL i=0; i<(LL)position.size(); i++)
                        fprintf(targetFile, "%lld ", position[i]);
                fprintf(targetFile, "\n");
        }
};
...
void readSourceFile(){
	...
	LL old_i = 0, new_i=0;////
        for (LL i=0; i<numberOfOpticalMap; i++){
                ...
                if (!opticalMap1[i].orientation){
                        LL tempMaxDis = opticalMap1[i].position[(LL)opticalMap1[i].position.size() - 1];
                        for (LL j=0; j<(LL)opticalMap1[i].position.size(); j++)
                                opticalMap1[i].position[j] = tempMaxDis - opticalMap1[i].position[j];
...
optAlignType *opticalMap;
distanceType *distancePair;
variantType *variant;
...
void init(){
	...
	variant = new variantType[3000000];
}
...
void readOpticalAlign(int chr, char* outputFileLocation){
	...
	while (fscanf(inputOptAlign, "%lld %s %lf %lf %lf %lf %lf %lld %lld %lld %lld %lld %s %lld", &opticalMap[cc].belongs, opticalMap[cc].mapId, &opticalMap[cc].score, &opticalMap[cc].confidence, &opticalMap[cc].fpr, &opticalMap[cc].fnr, &opticalMap[cc].alignRate, &tempLongLong, &opticalMap[cc].optStart, &opticalMap[cc].optEnd, &opticalMap[cc].refStart, &opticalMap[cc].refEnd, opticalMap[cc].hitEnum, &opticalMap[cc].numberOfSites) == 14){
		...
		for (LL i=0; i<opticalMap[cc].numberOfSites; i++){
			fscanf(inputOptAlign, "%lld", &opticalMap[cc].position[i]);
                ...
                if (distancePair[i].start!= ost || distancePair[i].end!=oed)//this locate the non-redundant reference regions (NRRR)
                {
                        ...
                        for (LL j = 0; j <= i-1; j++)//this search the possible sub-regions
                        {
                                ...
                                else//for each sub-region holding the same start point with NRRR, we explore the consecutive regions on the same molecule
                                {
                                        ...
                                        bool rep = true;
                                        for (LL k = j+1; distancePair[k].start<oed; k++)
                                        {
...
void correctOverlapVariant(){
	delete distancePair;
	delete opticalMap;
	variantType *tempVariant = new variantType[5000000];
	variantType *tempRealVariant = new variantType[5000000];
	...
	sort(tempRealVariant, tempRealVariant + tempRealCnt);
...
////////////////////////S2:find the cases without completely covering any others///////////////////////////////////////////////
	...
	sort(tempVariant, tempVariant + numberOfVariant);
	//numberOfVariant = tempRealCnt;
	for (LL i=0; i<numberOfVariant; i++)
		variant[i] = tempVariant[i];

	free(tempVariant);
	free(tempRealVariant);
}
